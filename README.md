...
| automation\_account\_runbook\_log\_contents | The list of desired content of the runbook. `NOTE: The Azure API requires a `publish\_content\_link` to be supplied even when specifying your own `content` & setting `content` to an empty string will revert the runbook to the `publish\_content\_link`.` | `list(string)` | <pre>[<br>  ""<br>]</pre> | no |
